...
namespace ft
{
	...
	class vector
	{
		...
		public:
			explicit vector (const allocator_type& alloc = allocator_type()) : _arr(u_nullptr), _size(0), _capacity(0), _alloc(alloc) {}
			...
			vector (const vector& x) : _arr(u_nullptr), _size(0), _capacity(0)
			{
				*this = x;
			}
